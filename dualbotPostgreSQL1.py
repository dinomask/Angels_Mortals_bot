...
def sendAngel(update: Update, context: CallbackContext, bot=mortalbot):
    ...
    try:
        if update.effective_message.reply_to_message:
                ...
                if update.effective_message.reply_to_message.from_user.username.lower() == playerName:
                    ...
                    if update.effective_message.sticker is not None:
                        list_of_entities = messagesdualbot.getMessageEntitybyYourALIAS_NoText(
                            ...
                            len(configdualbot.MORTAL_ALIAS))
                        bot.send_message(
                            text=f"|  In reply to:\n"
                                 f"|  Your {configdualbot.MORTAL_ALIAS}\n"
                                 f"|  {reply_message}",
            ...
            logger.info(f"{update.effective_message.chat.username} sent to {players[playerName].angel.username} a reply Text Message on Bot {context.bot.username}, {context.bot.first_name}.")
            ...
            logger.info(f"{update.effective_message.chat.username} sent to {players[playerName].angel.username} a Text Message on Bot {context.bot.username}, {context.bot.first_name}.")
        else:
            sendNonTextMessage(update.effective_message, bot, players[playerName].angel.chat_id, ANGEL_BOT_TOKEN)
        update.effective_message.reply_text(messagesdualbot.MESSAGE_SENT)
        return ANGEL
    except:
        update.effective_message.reply_text(messagesdualbot.STOPPED_BOT(configdualbot.ANGEL_ALIAS))
        return ANGEL
...
def sendMortal(update: Update, context: CallbackContext, bot=angelbot):
    ...
    try:
        ...
        update.effective_message.reply_text(messagesdualbot.STOPPED_BOT(configdualbot.MORTAL_ALIAS))
        return MORTAL
...
def main():
    ...
    dispatcherMortal.add_handler(CommandHandler("downloadchatids", savechatids_toCSV_command))
